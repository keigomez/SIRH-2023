...
namespace SIRH.Logica
{
    public class CComponentePresupuestarioL
    {
        ...
        public List<CBaseDTO> ListarComponentePresupuestario()
        {
            ...
            {
                CComponentePresupuestarioD intermedio = new CComponentePresupuestarioD(contexto);
                var resultado = intermedio.ListarComponentePresupuestario();

                if (resultado.Codigo > 0)
                {
                    var motivos = ((List<ComponentePresupuestario>)resultado.Contenido);
                    foreach (var item in motivos)
                    {
                        respuesta.Add(ConvertirDatosCompPresupuestarioADto(item));
                    }
                    return respuesta;
                }
                else
        ...
        public List<CBaseDTO> BuscarPresupuestos(CComponentePresupuestarioDTO componente, List<string> anios,
                                                   List<int> programas)
        {
            List<CBaseDTO> respuesta = new List<CBaseDTO>();
            try
            {
                ...
                List<ComponentePresupuestario> datos = new List<ComponentePresupuestario>();

                if (componente.AnioPresupuesto != null)
                {

                    var datosBD = ((CRespuestaDTO)intermedio.BuscarPresupuestos(datos, componente.AnioPresupuesto, "AnioPresupuesto"));

                    if (datosBD.Codigo > 0)
                    {
                        datos = (List<ComponentePresupuestario>)datosBD.Contenido;
                    }
                    else
                    {
                ...
                if (componente.Programa != null)
                {
                    var datosBD = ((CRespuestaDTO)intermedio.BuscarPresupuestos(datos, componente.Programa, "Programa"));
        ...
        public CBaseDTO ObtenerDecreto(int codDecreto)
        {
            ...
            try
            {
                ...
                var resultado = intermedio.ObtenerDecreto(codDecreto);

                if (resultado.Codigo > 0)
                {
                    respuesta = ConvertirDatosCompPresupuestarioADto(((ComponentePresupuestario)resultado.Contenido));
                    return respuesta;
                }
                else
                {
