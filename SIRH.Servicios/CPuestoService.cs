...
namespace SIRH.Servicios
{
    ...
    public class CPuestoService : ICPuestoService
    {
        ...
        {
            return (new CMovimientoPuestoL()).DetalleMovimiento(codMovimiento);
        }
        ...
        {
            CSubEspecialidadL logica = new CSubEspecialidadL();
            return logica.DescargarSubespecialidades(nombre);
        }
        public List<CBaseDTO> BuscarEscalaCategoria(int indCategoria)
        {
            CEscalaSalarialL logica = new CEscalaSalarialL();
            return logica.BuscarEscalaCategoria(indCategoria);
        }
        public List<CBaseDTO> ListarCategoriasEscalaSalarial()
        {
            CEscalaSalarialL logica = new CEscalaSalarialL();
            return logica.RetornarCategorias();
        }
        ...
        public List<List<CPresupuestoDTO>> BuscarPresupuestoParams(string codigo)
        {
            CPresupuestoL logica = new CPresupuestoL();
            return logica.DescargarPresupuestos(codigo);
        }

        public List<CProgramaDTO> ObtenerCodigosProgramas()
        {
            CPresupuestoL logica = new CPresupuestoL();
            return logica.DescargarCodigosProgramas();
        }
        ...
        {
            CMovimientoPuestoL logica = new CMovimientoPuestoL();
            return logica.BuscarMovimientoPuestosFiltros(codPuesto, numCedula, motivoM, fechaHasta, fechaDesde);
        }

        public List<List<CBaseDTO>> DescargarPerfilPuestoAccionesFuncionarioPP(string cedula)
        {
            CPuestoL logica = new CPuestoL();
            return logica.DescargarPerfilPuestoAccionesFuncionarioPP(cedula);
        }

        public List<CBaseDTO> BuscarCentroCostosParams(string centrocostos)
        {
            CUbicacionAdministrativaL logica = new CUbicacionAdministrativaL();
            return logica.BuscarCentroCostosList(centrocostos);
        }

        public List<CBaseDTO> BuscarCodigoPresupuestarioParams(string codigopresupuesto)
        {
            CPresupuestoL logica = new CPresupuestoL();
            return logica.BuscarCodigoPresupuestarioList(codigopresupuesto);
        }

        public List<CPuestoDTO> BuscarFuncionarioPuestoPuesto(string codPuesto, int codClase, int codEspecialidad, int codOcupacionReal, string estadoPuesto, string confianza, int codNivel)
        {
            CFuncionarioL logica = new CFuncionarioL();
            return logica.EnviarFuncionarioPuestoDetallePuesto(codPuesto, codClase, codEspecialidad, codOcupacionReal, estadoPuesto, confianza, codNivel);
        }

        public List<CBaseDTO> CargarPuestoPropiedad(string cedula)
        {
            return new CPuestoL().CargarPuestoPropiedad(cedula);
        }

        /*ORDEN DE MOVIMIENTO*/
        public List<CBaseDTO> DatosOrdenMovimiento(string cedula, string puesto)
        {
            return new COrdenMovimientoL().DatosOrdenMovimiento(cedula, puesto);
        }

        public CBaseDTO AgregarOrden(COrdenMovimientoDTO orden, COrdenMovimientoDeclaracionDTO declaracion)
        {
            return new COrdenMovimientoL().AgregarOrden(orden, declaracion);
        }

        public CBaseDTO AnularOrden(COrdenMovimientoDTO orden)
        {
            return new COrdenMovimientoL().AnularOrden(orden);
        }

        public CBaseDTO ActualizarOrden(COrdenMovimientoDTO orden)
        {
            return new COrdenMovimientoL().ActualizarOrden(orden);
        }
        public List<CBaseDTO> ObtenerOrden(int codigo)
        {
            return new COrdenMovimientoL().ObtenerOrden(codigo);
        }

        public List<CBaseDTO> BuscarOrdenes(COrdenMovimientoDTO orden, List<DateTime> fechas)
        {
            return new COrdenMovimientoL().BuscarOrdenes(orden, fechas);
        }

        public List<CBaseDTO> ListarOrdenEstados()
        {
            return new COrdenMovimientoL().ListarOrdenEstados();
        }

        public List<CBaseDTO> DatosUbicacionDetallePuesto(string codpuesto)
        {
            return new CPuestoL().DatosUbicacionDetallePuesto(codpuesto);
        }

        public List<CBaseDTO> BuscarSubespecialidadParam(int codigo, string descripcion)
        {
            return new CSubEspecialidadL().BuscarSubespecialidadParam(codigo, descripcion);
        }
        public List<CBaseDTO> UbicacionAdministrativaSeccion(int seccion)
        {
            return new CUbicacionAdministrativaL().UbicacionAdministrativaSeccion(seccion);
        }
        ...
        public CBaseDTO ActualizarDatosPuesto(CDetallePuestoDTO detallePuesto, CUbicacionAdministrativaDTO ubicacionAdmin, CUbicacionPuestoDTO ubicacionContrato,
            ...
            => new CPuestoL().ActualizarDatosPuesto(detallePuesto, ubicacionAdmin, ubicacionContrato, ubicacionTrabajo, puesto);
