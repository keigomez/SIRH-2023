//------------------------------------------------------------------------------
// <auto-generated>
//    Este código se generó a partir de una plantilla.
//
//    Los cambios manuales en este archivo pueden causar un comportamiento inesperado de la aplicación.
//    Los cambios manuales en este archivo se sobrescribirán si se regenera el código.
// </auto-generated>
//------------------------------------------------------------------------------

namespace SIRH.Datos
{
    using System;
    using System.Collections.Generic;
    
    public partial class TipoIncapacidad
    {
        public TipoIncapacidad()
        {
            this.RegistroIncapacidad = new HashSet<RegistroIncapacidad>();
        }
    
        public int PK_TipoIncapacidad { get; set; }
        public string DesIncapacidad { get; set; }
        public Nullable<int> FK_EntidadMedica { get; set; }
    
        public virtual EntidadMedica EntidadMedica { get; set; }
        public virtual ICollection<RegistroIncapacidad> RegistroIncapacidad { get; set; }
    }
}
